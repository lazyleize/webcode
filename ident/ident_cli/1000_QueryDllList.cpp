...
bool CIdentRelayApi::QueryDllList(CStr2Map& paramap,CStr2Map& resultmap,vector<CStr2Map>& vectmapArray,bool throwexp)
{
    CStr2Map inMap;
	...
	if(!paramap["file_name"].empty())
	{
		str += "|file_name:" + paramap["file_name"];
	}
	if(!paramap["version"].empty())
	{
		str += "|version:" + paramap["version"];
	}
	inMap["fields"] = str;
