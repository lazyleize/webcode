...
CRelayCall* GetIdentSingleInstance(int objtype)
{
...
CRelayCall* CIdentRelayApi::GetRelayObj(int request_type,int objtype)
{
    string svrip    = "";
    int    iPort    = 22000 ;
    int    iTimeOut= 2 ;
    ...
    CRelayCall *pRelayCall = GetIdentSingleInstance(objtype);
...
CRelayCall* CIdentRelayApi::GetRelayObj(const string & reqtype,int objtype)
{
    string svrip    = "";
    int    iPort    = 22000 ;
    int    iTimeOut= 2 ;
...
bool CIdentRelayApi::ParseResult(bool bCallRes,CRelayCall* m_pRelayCall,CStr2Map & resultMap,bool throwexp,bool xmlPrase)
{
    if(!bCallRes)
    {
        g_RuntimeGather.SaveLog(WARN,"[%s,%d]error[%s]",__FILE__,__LINE__,m_pRelayCall->GetLastError().c_str());
        if(throwexp)
        {
    ...
    if(resultMap["errorcode"] != "0000")
    {
    	...
    	//Tools::ConvertCharSet((char*)(resultMap["errormessage"].c_str()),buff,iDestLen,"GBK","utf-8");
...
void CIdentRelayApi::CheckParams(const char ** pParams,CStr2Map & paramMap,const int& iRequestType, bool bStrongCheck)
{
    ...
    if(bStrongCheck)
    {
        for(int iIndex=0; pParams[iIndex]!= NULL;iIndex++)
        {
            ...
            {
                WarnLog("%d 接口调用参数:%s为空值ֵ",iRequestType,pParams[iIndex]);
                throw(CTrsExp(LACK_OF_PARAM,"参数错误")) ;
            }
        }
    }
